...
function fetchPOIs(lat, lon, listing_id) {
  return new Promise(resolve => {
    chrome.runtime.sendMessage(
      { type: 'FETCH_POIS', lat, lon, airbnb_id: listing_id },
      resolve
    );
  });
}
...
function extractFromDataAttributes() {
  ...
  const lon = parseFloat(el.dataset.lng);
...
// ── Location section anchor detection ─────────────────────────────────────

function findAnchor() {
  // Airbnb uses data-section-id attributes on section containers
  const section = document.querySelector('[data-section-id="LOCATION_DEFAULT"]');
  if (section) return section;

  // Fallback: h2 with location section text (EN + FR)
...
function injectMapContainer(anchor, listing_id, backendBase) {
  ...
  const viewUrl = backendBase
    ? `${backendBase}/airbnb/${listing_id}`
    : `http://localhost:5010/airbnb/${listing_id}`;
  ...
  // Insert before the location section so the map appears above it
  const section = anchor.closest('section, [data-section-id]') || anchor.parentElement;
...
async function run() {
  ...
  if (link && response.backendBase) {
    link.href = `${response.backendBase}/airbnb/${listing_id}`;
  }
